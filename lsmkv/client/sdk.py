...
from storage.read_repair import newest_record
...
class LsmkvClient:
    ...
    async def get(self, key: str) -> Optional[bytes]:
        ...
        replica_records = self._records_from_responses(
            key,
            responses,
        )
        ...
        records = [
            record
            for _, record in replica_records
        ]
        ...
        stale = [r for r in records if (r.version, r.timestamp,)
        <
        (newest.version,newest.timestamp,)
        ]

        # Placeholder for repair
        _ = stale

        for addr, record in replica_records:
            if (
                record.version,
                record.timestamp,
            ) < (
    ...
    async def snapshot_create(
        ...
        """
        Create a snapshot on a server node.
        """
        addr = node_addr or self._nodes[0]

        resp = await self._conns[addr].send(
            {
                "cmd": "SNAPSHOT_CREATE",
                "name": name,
            }
        ...
        if not resp.get("ok"):
            raise ClientError(
                f"Snapshot create failed: {resp.get('error')}"
            )
    ...
    async def snapshot_list(
        ...
        if not resp.get("ok"):
            raise ClientError(
                f"Snapshot list failed: {resp.get('error')}"
            )
    ...
    async def snapshot_restore(
        ...
        """
        Restore a snapshot on a server node.
        """
        addr = node_addr or self._nodes[0]
        ...
        if not resp.get("ok"):
            raise ClientError(
                f"Snapshot restore failed: {resp.get('error')}"
            )
    ...
    async def snapshot_info(
        ...
        if not resp.get("ok"):
            raise ClientError(
                f"Snapshot info failed: {resp.get('error')}"
            )
    ...
    async def snapshot_delete(
        ...
        if not resp.get("ok"):
            raise ClientError(
                f"Snapshot delete failed: {resp.get('error')}"
            )
    ...
    async def _read_from_replicas(
        ...
        """
        Read the key from every reachable replica.
        """

        replicas = self._ring.get_replicas(
            key,
            self._rf,
        )
    ...
    async def _repair_replica(
        ...
        """
        Repair a stale replica with the newest record.

        Errors are ignored because read repair is best-effort.
        """
        try:
            await self._conns[addr].send(
                {
                    "cmd": "REPLICA_SET",
                    "key": key,
                    "value": record.value,
    ...
    def _records_from_responses(
        ...
        """
        Convert GET responses into Record objects.
        """
            ...
            records.append(
                (
                    ...
                    Record(
                        ...
                        tombstone=resp.get("tombstone", False),
                    ),
                )
            )
