    ...
    async def _dispatch(self, msg: dict) -> bytes:
        ...
        try:
            if cmd == "PING":
                ...
                # Record op BEFORE replication so metrics are captured even if
                # ReplicationError is raised (which would otherwise skip record_op).
                self._metrics.record_op("SET", asyncio.get_event_loop().time() - start)
                await self._replicate(msg)
                return encode_ok()
            
            elif cmd == "REPLICA_SET":
                record = Record(
                    key=msg["key"],
                    value=bytes(msg["value"]),
                    version=msg["version"],
                    ...
                    tombstone=msg.get("tombstone", False),
                )

                await self._engine.repair_set(record)
                self._metrics.record_read_repair()
                ...
                record = await self._engine.get_record(key)
                ...
                if record is None or record.tombstone:
                    ...
                    return encode_error("Key not found")
                ...
                await self._engine.snapshot_create(name)

                return encode_ok()
            
            elif cmd == "SNAPSHOT_LIST":
                snapshots = await self._engine.snapshot_list()
                ...
                await self._engine.snapshot_restore(name)

                return encode_ok()
            
            elif cmd == "SNAPSHOT_INFO":
                info = await self._engine.snapshot_info(
                    msg["name"],
                )
                ...
                await self._engine.snapshot_delete(
                    msg["name"],
                )
                ...
                # Flush the MemTable to a new SSTable immediately.
                await self._engine._flush_memtable()
                return encode_ok(message="MemTable flushed to SSTable")

            elif cmd == "COMPACT":
                # Force a compaction run on all levels that have data.
                def _do_compact():
                    comp = self._engine._compaction
                    reg = comp._registry
                    ran = False
                    for lvl in [0, 1, 2]:
                        ssts = reg.get_level(lvl)
                        if ssts:
                            comp._compact_level(lvl, ssts)
                            ran = True
                    return ran
                ran = await asyncio.to_thread(_do_compact)
                msg_txt = "Compaction complete" if ran else "Nothing to compact (all levels empty)"
                return encode_ok(message=msg_txt)
