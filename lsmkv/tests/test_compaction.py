...
def test_merge_no_duplicates(tmp_path):
    ...
    keys = [record.key for record in merged]
...
def test_newest_wins_on_duplicate(tmp_path):
    ...
    merged = {
        ...
        for record in _kway_merge([sst_old, sst_new])
    }
...
def test_merge_output_sorted(tmp_path):
    ...
    merged = [record.key for record in _kway_merge([sst1, sst2])]
...
def test_newer_timestamp_wins_same_version():
    r1 = Record(
        key="x",
        value=b"old",
        version=2,
